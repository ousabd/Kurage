...
df %>% 
  filter(side == "Avg") %>% 
  group_by(metric) %>% 
  summarise(across(c(GAITRite, NeuroSkin), sd)) %>% 
  rowwise() %>%
  mutate(sd = mean(c(GAITRite, NeuroSkin)), .keep = "unused") %>% 
  left_join(df.cor) %>% 
  filter(!is.na(correlation)) %>% 
  mutate(MDC95 = 1.96*sd*sqrt(1-correlation)) 
...
compute_abserror_bootci <- function(data, var.name, n = 1000) {
  bt <- boot::boot(data, statistic = mean.bootfn, R = n, var.name = var.name)
...
### Bootstrap median & CIs: signed error

```{r}
df.bootci.sgn <- df.error %>%
  filter(!grepl("phase",metric)) %>% 
  group_split(metric,side) %>%  # Split by group
  map_dfr(~ mutate(compute_abserror_bootci(., var.name = "error", n = 5000),
                   metric = unique(.$metric),
                   side = unique(.$side),
                   .before = 1))
...
### Bootstrap median & CIs: absolute error

Compute :

```{r}
df.bootci.abs <- df.error %>%
  filter(!grepl("phase",metric)) %>% 
  group_split(metric,side) %>%  # Split by group
  map_dfr(~ mutate(compute_abserror_bootci(., var.name = "abserror", n = 5000),
                   metric = unique(.$metric),
                   side = unique(.$side),
                   .before = 1))
```
